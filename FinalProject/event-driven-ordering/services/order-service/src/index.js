...
// Kafka
const kafka = new Kafka({ clientId: process.env.SERVICE_NAME || 'order-service', brokers: KAFKA_BROKERS });
const producer = kafka.producer();
const consumer = kafka.consumer({ groupId: (process.env.SERVICE_NAME || 'order-service') + '-group' });
...
async function handleEvent(event) {
  ...
  const svc = (process.env.SERVICE_NAME || 'order-service');
  try {
...
if ((process.env.SERVICE_NAME || 'order-service') === 'order-service') {
