...
// Kafka
const kafka = new Kafka({ clientId: process.env.SERVICE_NAME || 'payment-service', brokers: KAFKA_BROKERS });
const producer = kafka.producer();
const consumer = kafka.consumer({ groupId: (process.env.SERVICE_NAME || 'payment-service') + '-group' });
...
async function handleEvent(event) {
  ...
  const svc = (process.env.SERVICE_NAME || 'payment-service');
  try {
...
if ((process.env.SERVICE_NAME || 'payment-service') === 'order-service') {
