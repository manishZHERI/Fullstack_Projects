...
// Kafka
const kafka = new Kafka({ clientId: process.env.SERVICE_NAME || 'shipping-service', brokers: KAFKA_BROKERS });
const producer = kafka.producer();
const consumer = kafka.consumer({ groupId: (process.env.SERVICE_NAME || 'shipping-service') + '-group' });
...
async function start() {
  ...
  app.listen(PORT, () => log.info('shipping-service listening on ' + PORT));
}
...
async function handleEvent(event) {
  ...
  const svc = (process.env.SERVICE_NAME || 'shipping-service');
  try {
...
if ((process.env.SERVICE_NAME || 'shipping-service') === 'order-service') {
